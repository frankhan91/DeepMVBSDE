...
class SineBMNewDBDPSolver():
    ...
    def __init__(self, config, bsde):
        ...
        self.netz = FeedForwardNoBNSubNet(config, self.eqn_config.dim**2)
        self.nety = FeedForwardNoBNSubNet(config, self.eqn_config.dim)
        self.nety_target = FeedForwardNoBNSubNet(config, self.eqn_config.dim)
        tmp_x = np.random.normal(size=[1, self.bsde.dim+1])
        self.nety(tmp_x)
        self.nety_target(tmp_x)
        self.netz(tmp_x)
    ...
    def train(self):
        ...
        for step in range(self.opt_config.num_sweep):
            ...
            self.train_one_sweep(train_data)
            if step % 1 == 0:
                loss, _ = self.total_loss_fn(valid_data)
                y_init = np.mean(self.y_init.numpy()[0]**2)
                path_data = self.bsde.sample(self.eqn_config.N_simu, withtime=True)
                _, y_path = self.total_loss_fn(path_data)
                # always assuming the first dimension is time
                y_path_err = np.mean((y_path.numpy()-path_data[1][:, 1:])**2)
                elapsed_time = time.time() - start_time
                training_history.append([step, loss, y_init, y_path_err, elapsed_time])
                if self.net_config.verbose:
                    logging.info("step: %5u,    loss: %.4e, Y0: %.4e,   Y_path_err: %.4e,   elapsed time: %3u" % (
    ...
    def train_one_sweep(self, train_data):
        ...
        for t in range(self.bsde.num_time_interval-1, -1, -1):
            ...
            for _ in range(self.opt_config.num_iterations_perstep):
                if t > 0:
                    self.train_step_inter(train_data, t)
                else:
                    self.train_step_init(train_data)
            if t > 0:
                self.nety_weights[t] = self.nety.get_weights()
                self.netz_weights[t] = self.netz.get_weights()
                self.nety_target.set_weights(self.nety_weights[t])
    ...
    def local_loss_fn(self, inputs, t):
        ...
        if t == 0:
            ...
            z = self.netz(x[:, :, t]) / self.bsde.dim
        y_cross = [y, y_target]
        z = tf.reshape(z, (-1, dim, dim))
        drift_predict = self.bsde.drift_predict(t, y, all_one_vec)
        drift_true = tf.exp(-tf.reduce_sum(y**2, axis=-1, keepdims=True)/(dim + 2*time_stamp[t]))*(dim/(dim+2*time_stamp[t]))**(dim/2)
        y_next = y + self.bsde.delta_t * tf.sin(drift_predict - drift_true) + tf.matmul(z, dw[:, :, t:t+1])[:, :, 0]
        delta = y_next - y_target
        # use linear approximation outside the clipped range
        loss = tf.reduce_mean(
            tf.where(tf.abs(delta) < DELTA_CLIP, tf.square(delta),
    ...
    def total_loss_fn(self, inputs):
        loss = 0
        y_path = [None] * (self.bsde.num_time_interval+1)
        for t in range(self.bsde.num_time_interval-1, -1, -1):
            if t > 0:
                self.nety.set_weights(self.nety_weights[t])
                self.netz.set_weights(self.netz_weights[t])
            if t < self.bsde.num_time_interval-1:
                self.nety_target.set_weights(self.nety_weights[t+1])
            loss_tmp, y_cross_tmp = self.local_loss_fn(inputs, t)
            y_path[t] = y_cross_tmp[0]
            if t == self.bsde.num_time_interval-1:
                y_path[self.bsde.num_time_interval] = y_cross_tmp[1]
            loss += loss_tmp.numpy()
    ...
    def grad(self, inputs, t):
        with tf.GradientTape(persistent=True) as tape:
            loss, _ = self.local_loss_fn(inputs, t)
        if t == 0:
            grad = tape.gradient(loss, self.init_variables)
        else:
            grad = tape.gradient(loss, self.net_variables)
    ...
    @tf.function
    def train_step_inter(self, train_data, t):
        grad = self.grad(train_data, t)
        self.optimizer.apply_gradients(zip(grad, self.net_variables))

    @tf.function
    def train_step_init(self, train_data):
        grad = self.grad(train_data, 0)
        self.optimizer.apply_gradients(zip(grad, self.init_variables))
